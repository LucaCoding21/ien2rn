...
export default function PageHero({ label, heading, description, image, imageAlt, headingClassName, headingStyle }: PageHeroProps) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.to(".page-hero-label", { y: 0, autoAlpha: 1, duration: 0.4 })
        .to(".page-hero-heading", { y: 0, autoAlpha: 1, duration: 0.5 }, "-=0.4")
        .to(".page-hero-desc", { y: 0, autoAlpha: 1, duration: 0.4 }, "-=0.4")
        .to(
          ".page-hero-line",
  ...
  return (
    <section ref={sectionRef} className="relative pt-32 pb-6 sm:pt-32 sm:pb-12 md:pt-40 md:pb-16">
      <div className="max-w-[1400px] mx-auto px-5 sm:px-6 md:px-12">
        <div className={`flex flex-col ${image ? "lg:flex-row lg:items-center lg:gap-16" : ""}`}>
          <div className={image ? "lg:flex-1" : "max-w-3xl"}>
            ...
            <p className="page-hero-desc font-body text-sm sm:text-lg text-muted leading-relaxed max-w-2xl">
              {description}
            </p>
          </div>
        ...
        {/* Subtle accent line */}
        <div className="page-hero-line hidden sm:block h-px bg-gradient-to-r from-primary/25 via-secondary/20 to-transparent mt-12 origin-left" />
      </div>
    </section>
  );
}
